    ...
    uint32_t color )
{
    ...
    // bbox
    p->BoundingBox = irr::core::aabbox3df( pos.x-0.5f*size.x, pos.y-0.5f*size.y, pos.z,
    ...
    // vertices = 12 + 4 * tesselation
    p->Vertices.reserve( 12 + 4 * tesselation );
	 // p->Vertices.reallocate(12 + 4*tesselation);
    // p->Vertices.set_used(0);

    auto addVertex = [ p, &pos, color ] ( glm::vec3 const & P, float32_t u, float32_t v )
    {
        p->Vertices.push_back( irr::video::S3DVertex( pos.x + P.x, pos.y + P.y, pos.z + P.z, 0, 0, -1, color, u, v ) );
    };
    ...
    p->Indices.reserve( 30 + 4 * 3 * ( tesselation + 1 ) );
	 // p->Indices.reallocate(30 + 4*3*(tesselation+1));
    ...
    irrExt::SinCosTablef const table( tesselation, 0.0f, 0.5f * M_PI );

    //! topLeft BJG quarter circle
    // vertices += tesselation
    uint32_t k = p->Vertices.size();
    ...
    irr::video::SColor const & innerColor )
{
    ...
    // tables
    irrExt::SinCosTablef table( tesselation + 1 );
